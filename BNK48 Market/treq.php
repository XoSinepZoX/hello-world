...
<div class="managewebsiteoriginal-container">
  <h2>Send Trade Request</h2>
  	<?php
    $q = 'SELECT * FROM Listings LEFT JOIN Trade ON Listings.ItemID=Trade.ItemID WHERE Listings.ItemID='.$itemid.';';
    if ($res = $conn->query($q))
    {
        $row=$res->fetch_array();
    }
?>
    <div class = "myaccount-account-info">
        ...
        <div class = "myaccount-account-info-row"><div class = "bold">Trade for :</div> <div><?php echo($row['TradeMemName']);  ?></div></div><br>
        <div class = "myaccount-account-info-row"><div class = "bold">Set :</div> <div><?php echo($row['TradeSetName']);  ?></div></div><br>
        <div class = "myaccount-account-info-row"><div class = "bold">Style :</div> <div><?php echo($row['TradeStyle']);  ?></div></div>
    </div>
    <?php 
        $q='SELECT COUNT(*) AS cnt FROM listings WHERE Mode="Trading" AND Status="Active" AND UID='.$uid;
        //echo($q);
        $res=$conn->query($q);
        $row=$res->fetch_array();
        $cnt=$row['cnt'];
    ...
    <form method="POST" action="requestQ.php">
    	...
    	<table border="0">
            ...
            <?php
                $q = 'SELECT DateAdded,listings.ItemID,Username,Topic,Description,Pic,MemName,SetName,Style,Mode,Status,Price,TradeMemName,TradeSetName,TradeStyle,PIC2,PIC3 FROM Listings LEFT JOIN buysell ON Listings.ItemID=buysell.ItemID LEFT JOIN trade ON listings.ItemID=trade.ItemID LEFT JOIN users ON listings.UID=users.UID LEFT JOIN PicComp on Listings.ItemID=PicComp.ItemID WHERE listings.UID='.$uid.' AND Status = "Active" AND Mode="Trading" ORDER BY DateAdded DESC';
                //echo($q);
                if ($res = $conn->query($q))
                {
    	...
    	<div class="centera">
    		<br>
    		<input type="submit" name="sendt" value="SEND TRADE REQUEST" onclick = "alert_treq()">
    		<br>
    		<br>
